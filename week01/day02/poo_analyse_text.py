...
def getText():
    if len(sys.argv) != 2:
        print(
            "Qu'en un programme, un seul fichier tienne jusqu'à la fin le programme exécuté !"
        )
        sys.exit()
    else:
        ...
        if os.path.exists(file_path) and os.path.getsize(file_path) != 0:
            ...
            print(
                "Au commencement, le fichier doit existé et du texte il doit contenir."
            )
            sys.exit()
...
def wordCount():
    ...
    print(
        f"Pour votre texte créé, le créateur l'a doté de {charatersAll} caractères avec espace."
    )
    for x in text:
        if x != " ":
            ch += 1
    ...
    unikmots = set(mots)
    print(
        f"Sans redondance, le créateur a utilisé {len(unikmots)} unique pour façonner ce texte."
    )
    for x in unikmots:
        number = mots.count(x)
        motOften[x] = number
